...
func Connection(conf *cf.Config) (res Cache, err error) {
	var ICache Cache
	switch s := conf.Hosts.Cache.Driver; {
	case strings.ToLower(s) == "redis":
		ICache = NewRedisCache(redis.NewClient(&redis.Options{
			Addr: fmt.Sprintf("%s:%d", conf.Hosts.Cache.Address, conf.Hosts.Cache.Port),
			DB:   0,
		}))
	case strings.ToLower(s) == "memory":
		ICache = NewMemCache(&cache.Cache{})
	}
